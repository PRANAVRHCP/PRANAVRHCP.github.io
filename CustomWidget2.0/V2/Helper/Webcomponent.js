(function () {
 ...
 `<button type="button" id="newBTN" > Download Logs</button>` ;  
  ...
  class PerformanceHelper extends HTMLElement {
         ...
         $(document).ready(function(){          
          $('html').click(async function(event){
            ...
            if(sNo == 1)  
            {
              ...
              if(widgetmode === 1 && (event.target.tagName !== 'custom-dropdown' || event.target.tagName !== 'CUSTOM-DROPDOWN' ) )
              {
              setTimeout(function() 
              {              
      ...
      // When the mode is to download the log
      fireStepLogger()
      {
        console.log('step log initiated');
      }
